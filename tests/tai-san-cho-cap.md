...
| ~~17~~ | ~~**Ảnh chụp màn hình app thật**~~ — **ĐÃ CÓ 24/08/2026** | 4 ô video (Trang chủ, Tải về, Hướng dẫn) **+ khối ảnh app đầu trang chủ** | Ô video đang chờ: ảnh app thật, phủ tối 45%, nút play xám không bấm được. Khối đầu trang chủ (24/08/2026, thay cho bản dựng HTML cũ): cùng tấm ảnh nhưng **không phủ tối, không chữ đè lên** | xong — `public/anh/phan-mem-goi-hang-ma-qr-coclipnha.webp`, đường dẫn khai ở `src/lib/video.ts` (`ANH_POSTER_APP`) + bản nét 1920×1080 `...-1920.webp` (`ANH_POSTER_APP_2X`, dùng cho `srcset` và link "xem ảnh to"). ⛔ Ảnh đã **làm mờ tên/SĐT/địa chỉ khách** trên tem; bản gốc chưa che nằm ở `QR module/Screenshot app/` — đừng đăng bản đó. ⚠️ Ảnh này chụp lúc app ở trạng thái **Sẵn sàng** (chưa ghi hình, 0 video, chưa nhập tên nhân viên) — muốn ảnh đầu trang chủ kể chuyện "đang ghi hình" thì cần cấp thêm một ảnh chụp lúc băng đỏ hiện |
