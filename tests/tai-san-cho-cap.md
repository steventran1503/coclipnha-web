...
| 8a | **Video demo NGẮN** (quay màn hình app chạy một ca gói) | Trang chủ — khối ảnh app đầu trang | Chưa có mã: hiện ẢNH CHỤP MÀN HÌNH THẬT, **không** có nút play (không mời bấm vào chỗ chưa có gì); dòng chữ dưới ảnh nói "sẽ đặt ngay trong khung này". Điền mã là nút play đỏ tự hiện giữa ảnh | `src/lib/video.ts` — điền `VIDEO_DEMO` |
...
| ~~17~~ | ~~**Ảnh chụp màn hình app thật**~~ — **ĐÃ CÓ 24/08/2026** | 4 ô video (Trang chủ, Tải về, Hướng dẫn) **+ khối ảnh app đầu trang chủ** | Ô video đang chờ: ảnh app thật, phủ tối 45%, nút play xám không bấm được. Khối đầu trang chủ (24/08/2026, thay cho bản dựng HTML cũ) dùng **TẤM THỨ HAI**: ảnh chụp lúc ĐANG GHI HÌNH (băng đỏ + mã đơn + giờ cháy trên hình), **không phủ tối, không chữ đè lên** | xong — `public/anh/phan-mem-goi-hang-ma-qr-coclipnha.webp`, đường dẫn khai ở `src/lib/video.ts` (`ANH_POSTER_APP`) + bản nét 1920×1080 `...-1920.webp` (`ANH_POSTER_APP_2X`, dùng cho `srcset` và link "xem ảnh to"). ⛔ Ảnh đã **làm mờ tên/SĐT/địa chỉ khách** trên tem; bản gốc chưa che nằm ở `QR module/Screenshot app/` — đừng đăng bản đó. TẤM THỨ HAI (chủ dự án chọn và tự che 24/08/2026): `phan-mem-quay-video-ma-qr-van-don-coclipnha.webp` + bản cỡ gốc `-1671.webp`, khai ở `ANH_APP_DANG_GHI` / `ANH_APP_DANG_GHI_2X`; gốc `QR module/Screenshot app/Phần mềm quay video bằng mã QR vận đơn Coclipnha.jpg`. ⛔ Đã làm mờ khối người nhận, mã QR trên tem và mã đơn cuối tem; trong khung KHÔNG có mặt người. ⚠️ Ảnh gốc chỉ rộng 1672px nên không có bản 1920 |
